...
ObjectEnemyBlue::ObjectEnemyBlue(int iAppearance, int iOutTime, int iPosX, int iPosY, int iDestPosX, int iDestPosY, float fDestAngle,
	int iOutPosX, int iOutPosY, float fOutAngle)
{
	m_ComprehensiveType = df_COMPREHENSIVE_TYPE_ENEMY;
	m_Type = df_TYPE_OBJECT_ENEMY_BLUE;
	...
	m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_CENTER_1;
	m_iAnimeStart = df_SPRITE_ENEMY_BLUE_CENTER_1;
	m_iAnimeEnd = df_SPRITE_ENEMY_BLUE_CENTER_2;
	m_iAnimeCount = 0;
...
ObjectEnemyBlue::~ObjectEnemyBlue()
{
}

bool ObjectEnemyBlue::Action()
{
	int iTime;
	int iPattern;
	int iRand;
	...
	if (!m_bOnFlag) {
		...
		// 이동 처리
		if (Move(m_fDestPosX, m_fDestPosY, m_fDestAngle, df_SPEED_ENEMY_BLUE_MOVE)) {
			// 배치 완료
			m_bOnFlag = true;
	...
	if (m_bOutFlag) {
		// 이동 처리
		if (Move(m_fOutPosX, m_fOutPosY, m_fOutAngle, df_SPEED_ENEMY_BLUE_MOVE)) {
			// 퇴장 완료
			return true;
		}
		return false;
	...
	// 다음 이동 장소로 가거나
	//-------------------------------------------
	if (Move(m_fDestPosX, m_fDestPosY, 0, df_SPEED_ENEMY_BLUE_STAND, true)) {
		if (m_Action != STATE_STAND) {
			m_Action = STATE_STAND;
			m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_CENTER_1;
			m_iAnimeStart = df_SPRITE_ENEMY_BLUE_CENTER_1;
...
//------------------------------------
bool ObjectEnemyBlue::Animation(int iAnimeDelay)
{
...
bool ObjectEnemyBlue::Move(float fDestPosX, float fDestPosY, float fAngle, float fSpeed, bool bOption)
{
	...
	if (fDest < fSpeed) {
		...
		if (vx2 > 4) {
			if (m_Action != STATE_RR) {
				m_Action = STATE_RR;
				m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_RIGHT_2_1;
				m_iAnimeStart = df_SPRITE_ENEMY_BLUE_RIGHT_2_1;
				m_iAnimeEnd = df_SPRITE_ENEMY_BLUE_RIGHT_2_2;
				m_iAnimeCount = 0;
			}
		}
		// STATE_R
			...
			if (m_Action != STATE_R) {
				m_Action = STATE_R;
				m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_RIGHT_1_1;
				m_iAnimeStart = df_SPRITE_ENEMY_BLUE_RIGHT_1_1;
				m_iAnimeEnd = df_SPRITE_ENEMY_BLUE_RIGHT_1_2;
				m_iAnimeCount = 0;
			}
			...
			if (m_Action != STATE_LL) {
				m_Action = STATE_LL;
				m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_LEFT_2_1;
				m_iAnimeStart = df_SPRITE_ENEMY_BLUE_LEFT_2_1;
				m_iAnimeEnd = df_SPRITE_ENEMY_BLUE_LEFT_2_2;
				m_iAnimeCount = 0;
			}
		}
		else if (vx2 < -1) {
			if (m_Action != STATE_L) {
				m_Action = STATE_L;
				m_iSpriteIndex = df_SPRITE_ENEMY_BLUE_LEFT_1_1;
				m_iAnimeStart = df_SPRITE_ENEMY_BLUE_LEFT_1_1;
				m_iAnimeEnd = df_SPRITE_ENEMY_BLUE_LEFT_1_2;
				m_iAnimeCount = 0;
			}
		}
		else {
