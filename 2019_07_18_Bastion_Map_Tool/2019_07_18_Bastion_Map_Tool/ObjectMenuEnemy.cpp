...
ObjectMenuEnemy::ObjectMenuEnemy(int iPosX, int iPosY, ObjectMenu * MenuPtr)
{
	// 위치
	m_dX = iPosX;
	m_dY = iPosY;
	// 스프라이트 번호
	m_iSpriteIndex = df_SPRITE_MENU_ENEMY;
	// 오브젝트 범위
...
ObjectMenuEnemy::~ObjectMenuEnemy()
{
}

bool ObjectMenuEnemy::Action()
{
	return false;
}

void ObjectMenuEnemy::Draw()
{
	g_cSprite->DrawImage(m_iSpriteIndex, m_dX, m_dY, g_bypDest, g_iDestWidth, g_iDestHeight, g_iDestPitch, false);
}

bool ObjectMenuEnemy::Click()
{
	int iMouseX;
	int iMouseY;
	...
	if (m_dLeft < iMouseX && iMouseX < m_dRight &&
		...
		m_MenuPtr->m_ChangeMenu = FOCUS_MENU_ENEMY;
...
void ObjectMenuEnemy::Move(int iMoveValueX, int iMoveValueY)
{
