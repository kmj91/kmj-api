...
/***********************************************************
	DrawIncreaseSprite
	특정 메모리 위치에 스프라이트를 출력(칼라키, 클리핑 처리)
	스프라이트 크기 늘려서 출력
************************************************************/
void CSpriteDib::DrawIncreaseSprite(int iSpriteIndex, int iDrawX, int iDrawY, BYTE * bypDest, int iDestWidth,
	int iDestHeight, int iDestPitch, int iIncreaseValue, bool bCameraOption)
{
	//-----------------------------------------------------------------
	// 최대 스프라이트 개수를 초과 하거나, 로드되지 않는 스프라이트라면 무시
	//-----------------------------------------------------------------
	...
	int iCountY;
	int iCountX;
	int iSpriteHeight = stpSprite->iHeight * iIncreaseValue;		// 스프라이트 크기를 원본보다 늘려줌;
	int iSpriteWidth = stpSprite->iWidth * iIncreaseValue;			// 스프라이트 크기를 원본보다 늘려줌;
	int iSpritePitch = stpSprite->iPitch;
	int iSpriteCenterX = stpSprite->iCenterPointX * iIncreaseValue;	// 중점 좌표도 늘어남
	int iSpriteCenterY = stpSprite->iCenterPointY * iIncreaseValue; // 중점 좌표도 늘어남
	int iSpriteColorByte = stpSprite->iColorByte;
	DWORD *dwpSprite = (DWORD *)stpSprite->bypImage;
	DWORD *dwpDest = (DWORD *)bypDest;
	BYTE *bypTempSprite;
	...
	if (bCameraOption) {
		...
		if (iDrawY < m_iCameraPosY) {
			...
			if (iSpriteHeight > 0) {
				...
				iSpriteCntY = (clippingY % iIncreaseValue) + 1;
				// 실제 잘려지는 값은 원본 크기 값 비율로 바꿔서 잘라줌
				dwpSprite = (DWORD *)((BYTE *)dwpSprite + iSpritePitch * (clippingY / iIncreaseValue));
			}
			iDrawY = m_iCameraPosY;
		}
		...
		if (iDrawX < m_iCameraPosX) {
			...
			if (iSpriteWidth > 0) {
				// 나머지가 있으면 그 부분부터 카운트 함
				iSpriteCntX = (clippingX % iIncreaseValue) + 1;
				// 실제 잘려지는 값은 원본 크기 값 비율로 바꿔서 잘라줌
				dwpSprite = dwpSprite + (clippingX / iIncreaseValue);
			}
			iDrawX = m_iCameraPosX;
		}
...
/******************************************
	...
	스프라이트 특정영역 잘래내고 출력
*******************************************/
void CSpriteDib::DrawCutOutSprite(int iSpriteIndex, int iDrawX, int iDrawY, BYTE * bypDest, int iDestWidth,
...
/*********************************
	DrawCutOutIncreaseImage
	이미지 특정영역 잘래내고 출력 및
	이미지 크기 늘려서 출력
**********************************/
void CSpriteDib::DrawCutOutIncreaseImage(int iSpriteIndex, int iDrawX, int iDrawY, BYTE * bypDest, int iDestWidth,
	int iDestHeight, int iDestPitch, int iIncreaseValue, RECT SpriteCutOutRect, bool bCameraOption)
{
	//-----------------------------------------------------------------
	// 최대 스프라이트 개수를 초과 하거나, 로드되지 않는 스프라이트라면 무시
	//-----------------------------------------------------------------
	...
	if (iSpriteHeight < SpriteCutOutRect.top) {
		...
		iSpriteHeight = iSpriteHeight - SpriteCutOutRect.top;
		// 나머지가 있으면 그 부분부터 카운트 함
		iSpriteCntY = (SpriteCutOutRect.top % iIncreaseValue) + 1;
		// 실제 잘려지는 값은 원본 크기 값 비율로 바꿔서 잘라줌
		dwpSprite = (DWORD *)((BYTE *)dwpSprite + iSpritePitch * (SpriteCutOutRect.top / iIncreaseValue));
	}
	...
	if (iSpriteWidth < SpriteCutOutRect.left) {
		...
		iSpriteWidth = iSpriteWidth - SpriteCutOutRect.left;
		// 나머지가 있으면 그 부분부터 카운트 함
		iSpriteCntX = (SpriteCutOutRect.left % iIncreaseValue) + 1;
		// 실제 잘려지는 값은 원본 크기 값 비율로 바꿔서 잘라줌
		dwpSprite = dwpSprite + (SpriteCutOutRect.left / iIncreaseValue);
	}
...
// 사각형 테두리 그리기
void CSpriteDib::DrawRectangleBorder(DWORD dwRGBColor, int iBorderValue, int iDrawX, int iDrawY, int iWidth, int iHeight,
	BYTE * bypDest, int iDestWidth, int iDestHeight, int iDestPitch, bool bCameraOption)
{
	int iCountY;
	int iCountX;
	DWORD *dwpDest = (DWORD *)bypDest;
	...
	//-----------------------------------------------------------------
	// 인자로 받은 가로 세로 길이 만큼 사각형 테두리를 그린다
	//-----------------------------------------------------------------
	for (iCountY = 0; iHeight > iCountY; ++iCountY)
	{
		for (iCountX = 0; iWidth > iCountX; ++iCountX)
		{
			if (((iCountY >= 0 && iCountY < iBorderValue) || (iCountY >= iHeight - iBorderValue && iCountY < iHeight)) ||
				((iCountX >= 0 && iCountX < iBorderValue) || (iCountX >= iWidth - iBorderValue && iCountX < iWidth))) {
				*dwpDest = dwRGBColor;
			}
