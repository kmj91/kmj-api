...
ListTrigger::ListTrigger(int iPosX, int iPosY, int iWidth, int iHeight, int iScrollBarWidth)
{
	ObjectUI * newObject;
...
bool ListTrigger::Action()
{
	return false;
}

void ListTrigger::Draw()
{
}

bool ListTrigger::Click()
{
	int iSize;
	int iMouseX;
	int iMouseY;
...
void ListTrigger::Move(int iMoveValueX, int iMoveValueY)
{
	int iCnt;
	int iSize;
	ObjectUI **ObjectArray;
...
void ListTrigger::AddElementListPtr(st_UIElementInfo stElementListInfo)
{
	m_ElmentInfoVector->push_back(stElementListInfo);
}

void ListTrigger::ScrollMove(int iMoveValueY)
{
	int iCnt;
	int iSize;
	ObjectUI **ObjectArray;
...
// 메뉴의 리스트 영역을 교체 한다
void ListTrigger::ChangeElementList(int iFocusMenu)
{
	int iSize;
	int iCnt;
	...
	double dTemp;
	st_UIElementInfo *stpElemenInfoArray;		// 리스트 요소들 배열
	...
	while (iCnt < iSize) {
		...
		switch (stpElemenInfoArray[iCnt].iListType) {
		case df_FOCUS_TRIGGER_MENU_TRIGGER_LIST:
			//--------------------------------------
			// 트리거 리스트
			// 추가 요소 없음
			// 트리거 리스트 목록만 출력하면 됨
			...
			while (iCnt < iSize) {
				...
				// 가로 길이 고정 (리스트 스크롤바를 뺀 가로 길이)
				ObjectElementArray[iCnt]->m_iWidth = m_iElementListWidth - 5;

				iElementWidth = ObjectElementArray[iCnt]->m_iWidth;
				iElementHeight = max(iElementHeight, 5 + ObjectElementArray[iCnt]->m_iHeight);
